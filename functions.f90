subroutine evalfunc(x,DIM,fct,ifid,flag,f,df,d2f,v)
  ...
  real*8 :: x(DIM),f,df(DIM),d2f(DIM,DIM),scal,scal2,prd,time2
  real*4 :: time
...
!        call random_seed(nseed)
  ...
  if (reusesamples.eq.1) then
     ...
     if (flag.le.3) write(97,100) Cstat,(x(j)*(DS(2,j)-DS(1,j))+DS(1,j),j=1,dim),f,&
          (df(j)/(DS(2,j)-DS(1,j)),j=1,dim),((d2f(k,j)/(DS(2,j)-DS(1,j))&
          /(DS(2,k)-DS(1,k)),j=1,dim),k=1,dim) 
     if (flag.gt.3) write(97,100) Cstat,(x(j)*(DS(2,j)-DS(1,j))+DS(1,j),j=1,dim),f,&
          (df(j)/(DS(2,j)-DS(1,j)),j=1,dim),(v(j),j=1,dim),(d2f(k,1),k=1,dim)
     close(97)
