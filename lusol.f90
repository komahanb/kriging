      subroutine LUDecomposition(mode,isize,Rij,Rinv,det,detlog,verr)
      use LU
...
! mode=1 Try to Cholesky_Decomp Firstly, then try to LU Decomp if failure
      ...
      double precision, dimension(isize,isize), intent(in)  :: Rij
      double precision, dimension(isize,isize), intent(out) :: Rinv
      double precision, intent(out) :: det,detlog,verr
!     double precision, allocatable, dimension(:,:) :: ALU,DL,DU,TMP
...
!     end if
      ...
      if(mode.eq.1)then
        ifail = 1
        call CB_DECOMP(ideb,IDIM,verr,ifail)
      else
        ifail = 1
      end if
      ...
      if(ifail.eq.0)then
        ...
        call LU_DECOMP(ideb,IDIM,verr) !ALU,DL,DU,TMP,verr)
        if(ideb.ge.1) &
        write(*,'(a,e15.5)') &
        ...
        call LU_INVERSION(ideb,IDIM,verr) !ALU,DL,DU,TMP,verr)
      end if
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      subroutine LU_INVERSION(ideb,IDIM,verr) !ALU,DL,DU,TMP,verr)
      use LU
      implicit none
...
!     :: TMP
      double precision, allocatable, dimension(:,:) :: DLinv,DUinv
...
!                write(*,*)(ALU(k,k),k=0,IDIM-1)
                 stop'DU(i,j) = 0.d0'
              end if
              DUinv(i,j) = 1.d0 / DU(i,j)
           else
              if(DU(i,i).eq.0.d0)stop'DU(i,i)'
              vsum = 0.d0
              do k= IDIM-1,i+1,-1
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      subroutine LU_DECOMP(ideb,IDIM,verr) !ALU,DL,DU,TMP,verr)
      use LU
      implicit none
      integer, intent(in) :: IDIM,ideb
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      subroutine CB_DECOMP(ideb,IDIM,verr,ifail) 
      use LU
      implicit none
      integer, intent(in) :: IDIM,ideb
      double precision, intent(out) :: verr
